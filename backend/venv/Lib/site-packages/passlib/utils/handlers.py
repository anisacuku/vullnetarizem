...
class BackendMixin(PasswordHash):
    ...
    .. classmethod:: _load_backend_{name}(dryrun=False)
        ...
        is needed to enable the .  This may include importing modules, running tests,
        ...
        :param dryrun:
            ...
            switching the class over to the new .
    ...
    def get_backend(cls):
        """
        Return name of currently active .
        if no backend has been loaded, loads and returns name of default .
    ...
    def has_backend(cls, name="any"):
        """
        Check if support is currently available for specified .
    ...
    def set_backend(cls, name="any", dryrun=False):
        ...
        :arg name:
            ...
            * ``"any"`` -- use current backend if one is loaded,
              otherwise load the first available .

            * ``"default"`` -- use the first available .

            * any string in :attr:`backends`, loads specified .

        :param dryrun:
            If True, this perform all setup actions *except* switching over to the new .
    ...
    def _set_backend(cls, name, dryrun):
        ...
        handles actual loading of specified .
    ...
    def _stub_requires_backend(cls):
        """
        helper for subclasses to create stub methods which auto-load .
        """
        if cls.__backend:
...
class HasManyBackends(BackendMixin, GenericHandler):
    ...
    .. classmethod:: _calc_checksum_{name}
        ...
        for a given . it will only be called if the backend has
